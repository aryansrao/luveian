...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function setupUIInteractions() {
    // Consolidated handler for overlay UI interactions
    ...
    // Setup click suggestion auto-hide on mobile
    setupClickSuggestion();
    ...
    setTimeout(() => {
      ...
      setupProjectModal();
    }, isLowPerfMode ? 1000 : 300);
  }
  
  // Setup all overlay interactions (open, close, etc.)
  function setupOverlayInteractions() {
    ...
    setTimeout(() => {
      logoContainer.querySelector('.logo').style.transform = 'scale(1.2)';

      setTimeout(() => {
        logoContainer.querySelector('.logo').style.transform = 'scale(1)';
      }, 600);
    }, 500);
    ...
    dockButtons.forEach(button => {
      button.addEventListener('click', function() {
        const overlayName = this.getAttribute('data-overlay');
        const overlayId = `${overlayName}-overlay`;
        const overlay = document.getElementById(overlayId);
        
        // Check if this button is already active
        if (this.classList.contains('active')) {
          // If same button clicked again, close the overlay
          closeOverlay(overlay, this);
        } else {
          // Close any open overlay first
          closeAllOverlays();
          
          // Open the new overlay
          openOverlay(overlay, this);
        }
      });
    });
    ...
    closeButtons.forEach(button => {
      button.addEventListener('click', function() {
        ...
        const button = findButtonForOverlay(overlay);
    ...
    document.addEventListener('click', function(event) {
      // Check for company overlay
      if (companyOverlay && companyOverlay.classList.contains('active') &&
          !event.target.closest('#company-overlay') &&
          !event.target.closest('.logo-container')) {
        closeOverlay(companyOverlay);
      }
      
      // Check for other overlays
      const activeOverlay = document.querySelector('.content-overlay.active:not(#company-overlay)');
      if (activeOverlay &&
          !event.target.closest('.content-overlay') &&
          !event.target.closest('.dock') &&
          !event.target.closest('.logo-container')) {
        const button = findButtonForOverlay(activeOverlay);
        closeOverlay(activeOverlay, button);
      }
    });
    
    // Logo click opens company overlay
  ...
  // Find the dock button for a given overlay
  function findButtonForOverlay(overlay) {
    if (!overlay || !overlay.id) return null;
    
    const overlayName = overlay.id.replace('-overlay', '');
    return document.querySelector(`.dock > button[data-overlay="${overlayName}"]`);
  }
  
  // Open an overlay and optionally activate its button
  function openOverlay(overlay, button = null) {
    if (!overlay) return;
    ...
    // Update the URL hash - preserve the original hash that was used to open the overlay
    const currentHash = window.location.hash.substring(1);
    if (!currentHash) {
      const overlayName = overlay.id.replace('-overlay', '');
      history.replaceState(null, null, `#${overlayName}`);
    }
  ...
  // Close an overlay and deactivate its button
  function closeOverlay(overlay, button = null) {
    if (!overlay) return;
  ...
  // Close all active overlays and deactivate all buttons
  ...
  // Navigate to the overlay specified by the URL hash
  function navigateToHash() {
    const hash = window.location.hash.substring(1);
    if (!hash) return;
    
    // Map of possible hash values to overlay IDs
    const hashMap = {
      'works': 'Works-overlay',
      'Works': 'Works-overlay', // Keep both cases to maintain case sensitivity
      'designs': 'brand-overlay', // Keep the original 'designs' hash without changing it
      'brand': 'brand-overlay',
      'software': 'software-overlay',
    ...
    // Try to find a matching overlay
    let overlayId = hashMap[hash];
    
    // If no match found, try the hash directly with '-overlay' appended
    if (!overlayId) {
      overlayId = `${hash}-overlay`;
    }
  ...
  // Setup CTA buttons to open different overlays
  function setupCTAButtons() {
    document.querySelectorAll('.cta-button[data-overlay]').forEach(button => {
      button.addEventListener('click', function() {
        const targetOverlayName = this.getAttribute('data-overlay');
        const targetOverlayId = `${targetOverlayName}-overlay`;
        ...
        const targetButton = document.querySelector(`.dock > button[data-overlay="${targetOverlayName}"]`);
        openOverlay(targetOverlay, targetButton);
      });
    });
  }
